...
DRDS除了将后端数据库聚合成一个逻辑库，提供数据垂直拆分、水平拆分、读写分离等功能外，还做了很多增强，具备功能比较齐全的分布式查询/执行/优化引擎，提供完善的分布式事务管理方案，等等，DRDS已经不是一个简单的数据库代理/分库分表中间件，已经成为一个分布式数据库引擎。

可以将后端数据库看作一个高级版存储引擎，DRDS则可以看作数据库的查询/执行引擎，DRDS集群和后端数据库集群组成一个分布式数据库产品。
...
- 2019年GTS的开源版[Seata](http://seata.io/zh-cn/)发布，不仅在阿里云，在企业内部也可以用上GTS了；
...
#### 全局二级索引 GSI
概念参考[DRDS全局二级索引](https://help.aliyun.com/document_detail/142733.html)。在[Mycat分库分表](https://github.com/liuzhibin-cn/my-demo/blob/master/docs/Sharding-Mycat-Overview-Quickstart.md)中也创建了自定义索引表，在应用代码中维护。DRDS支持全局二级索引GSI，自动维护：<br />
