...
#echo "> Wait ZipKin to be READY"
#while :
#do
#	COUNT=`kubectl get statefulset | grep pub-zipkin | grep '1/1' -c`
#	if [ "$COUNT" = "1" ]; then
#		echo "> ZipKin is READY now"
#		break
#	fi
#	sleep 3
#done
...
while :
do
	COUNT=`kubectl get deployment | grep web-shop | grep '1/1' -c`
	if [ "$COUNT" = "1" ]; then
